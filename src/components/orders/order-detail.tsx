    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-xs text-gray-400">by {order.createdBy.name}</p>
        </div>

        {/* Exchange Rate */}
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-gray-700">Exchange Rate</h2>
          {order.exchangeRate ? (
            <>
        ...
        {/* Totals */}
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-gray-700">Order Totals</h2>
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500">Revenue (JPY)</span>
              <span className="font-medium">{formatJpy(Number(order.totalJpyPrice))}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Revenue (THB)</span>
              <span className="font-medium">{formatThb(Number(order.totalThbPrice))}</span>
            </div>
            <div className="border-t border-gray-100 pt-1.5 flex justify-between">
              <span className="font-medium text-green-700">Profit (JPY)</span>
              <span className="font-bold text-green-600">
                {formatJpy(Number(order.totalProfitJpy))}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium text-green-700">Profit (THB)</span>
              <span className="font-bold text-green-600">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase text-gray-500">Qty</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">Cost (JPY)</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">Price (JPY)</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">Profit (JPY)</th>
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">Profit (THB)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {order.items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3.5 text-center text-sm text-gray-700">{item.quantity}</td>
                  ...
                  <td className="px-4 py-3.5 text-right text-sm font-medium text-green-600">
                    {formatJpy(Number(item.profitJpy))}
                  </td>
                  <td className="px-4 py-3.5 text-right text-sm text-green-600">
            ...
            <tfoot>
              <tr className="border-t border-gray-200 bg-gray-50/80 font-medium">
                <td className="px-4 py-3.5 text-sm text-gray-700" colSpan={2}>
            ...
            <div key={item.id} className="rounded-xl border border-gray-100 bg-white p-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div>
                  ...
                  <p className="font-medium">{formatJpy(Number(item.unitJpyPrice))}</p>
                </div>
                <div>
                  <p className="text-gray-400">Profit (JPY)</p>
                  <p className="font-medium text-green-600">{formatJpy(Number(item.profitJpy))}</p>
                </div>
                <div>
                  <p className="text-gray-400">Profit (THB)</p>
                  <p className="font-medium text-green-600">{formatThb(Number(item.profitThb))}</p>
                </div>
              </div>
            </div>
          ))}
